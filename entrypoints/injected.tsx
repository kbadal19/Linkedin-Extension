...
// SVG as a React component in JSX
const GenerateIcon = () => {
  return (
    <svg
      width="24"
...
const InjectedScript = () => {
  ...
  useEffect(() => {
    const pollForMessageBox = setInterval(() => {
      const messageBox = document.querySelector(
        '.msg-form__contenteditable[contenteditable="true"]'
      ) as HTMLElement;

      if (messageBox) {
        clearInterval(pollForMessageBox);

        const injectGenerateIcon = () => {
          const existingIcon = messageBox.querySelector(".generate-icon");
          if (!existingIcon) {
            const generateIconContainer = document.createElement("div");
            generateIconContainer.className = "generate-icon";
            generateIconContainer.style.position = "absolute";
            generateIconContainer.style.cursor = "pointer";
            generateIconContainer.style.right = "10px";
            generateIconContainer.style.bottom = "10px";
            generateIconContainer.style.width = "24px";
            generateIconContainer.style.height = "24px";
            generateIconContainer.title = "Generate";

            messageBox.style.position = "relative";
            messageBox.appendChild(generateIconContainer);

            // Instead of injecting the SVG as HTML, use ReactDOM to render JSX
            ReactDOM.render(<GenerateIcon />, generateIconContainer);

            generateIconContainer.addEventListener("click", () => {
              openModal(); // Open the modal directly
            });
          }
        };

        const removeGenerateIcon = () => {
          const generateIcon = messageBox.querySelector(".generate-icon");
          if (generateIcon) {
            generateIcon.remove();
          }
        };

        messageBox.addEventListener("focus", injectGenerateIcon);
        messageBox.addEventListener("blur", removeGenerateIcon);
      }
    }, 1000);

    return () => clearInterval(pollForMessageBox); // Cleanup the interval on unmount
  }, []);
    ...
    <>
      {isModalOpen &&
        ReactDOM.createPortal(
          ...
          document.body // Overlay the modal on the body
        )}
    </>
  );
};
...
export default defineUnlistedScript(() => {
  const injectedComponent = <InjectedScript />;
  const rootElement = document.createElement("div"); // Create a new div for your injected component
  document.body.appendChild(rootElement);

  const root = createRoot(rootElement); // Use createRoot instead of ReactDOM.render
  root.render(injectedComponent); // Render the component
});
